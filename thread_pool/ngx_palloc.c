...
ngx_create_pool(size_t size)
{
    ...
    /* Nginx会分配一大块内存，其中内存头部存放ngx_pool_t本身内存池的数据结构
       ngx_pool_data_t p->d 存放内存池的数据部分（适合小于p->max的内存块存储）
       p->large 存放大内存块列表
       p->cleanup 存放可以被回调函数清理的内存块（该内存块不一定会在内存池上面分配）
     */
    p->d.last = (u_char *) p + sizeof(ngx_pool_t);  // 内存开始地址，指向ngx_pool_t结构体之后数据起始位置
...
ngx_reset_pool(ngx_pool_t *pool)
{
    ...
    /* 循环重新设置内存池data区域的p->d.last; data区域数据并不擦除 */
...
ngx_palloc_small(ngx_pool_t *pool, size_t size, ngx_uint_t align)
{
    ...
    */
    ...
    /* 如果没有缓存池空间可以容纳大小为size内存块，则需要重新申请一个缓存池pool节点 */
...
ngx_palloc_block(ngx_pool_t *pool, size_t size)
{
    ...
    /* 缓存池的pool数据结构会挂载子节点的ngx_pool_t数据结构
       ...
       每添加一个子节点，p->d.dailed就会+1，当添加超过4个子节点的时候
...
/*
 ...
 ngx_pool_t中的cleanup字段管理者一个特殊的链表，该链表的每一项都记录着一个特殊的需要释放的资源
...
ngx_pool_cleanup_add(ngx_pool_t *p, size_t size)
{
    ...
    printf("add cleanup: %p", c);

    return c;
}

/* 清除p->cleanup链上的内存块（主要是文件描述符） 
...
ngx_pool_delete_file(void *data)
{
    ...
    printf("file cleanup: fd:%d %s", c->fd, c->name);
